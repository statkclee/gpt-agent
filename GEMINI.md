...
| 이 값을 그대로 사용하면 | -1을 그대로 사용하면 |
| 이러한 특성 덕분에 | 시퀀스라는 특성 덕분에 |
| 이 패턴을 사용한다 | 계수기 패턴을 사용한다 |
| 이처럼 조합하면 | `find()`와 슬라이싱을 조합하면 |
| 이 특성을 활용한다 | 단락 평가 특성을 활용하면... |
| 이 프로그램은 | 위 프로그램은 |

**제거할 AI 특성 표현**:

1. **"이 + 명사" 지시 대명사** (최우선):
   - ❌ "이 메서드는", "이 값을", "이 문제를", "이처럼", "이러한"
   - ✅ 구체적 명사 사용: 메서드명, 변수명, 개념명 직접 명시
...
## 챕터 윤문 작업 가이드 (Python for Informatics 기반)

### 작업 목표

Python for Informatics 원서를 직역한 챕터들을 자연스러운 한국어 교재로 변환한다.
...
- **예시**: 12장 파일 - 10개 절 → 7개 절로 통합

#### 2. 디버깅 섹션 강화
- **위치**: 각 장 후반부에 별도 섹션으로 배치
- **내용**: 해당 장 주제와 관련된 실전 디버깅 팁
- **callout-note**: 개념 설명 (예: 공백 문자란?)
- **callout-warning**: 주의사항 (예: 운영체제별 줄 끝 문자)
- **SVG 시각화**: 디버깅 핵심 개념 시각화

#### 3. SVG 다이어그램 추가
- **전체 조망 SVG**: 장 도입부에 해당 장 학습 내용 개요
- **핵심 개념 SVG**: 주요 지점마다 개념 시각화
- **Tufte 스타일**: grayscale, Palatino 폰트, 최소 장식
- **파일명 규칙**: `{챕터명}-{주제}.svg` (예: `file-handler.svg`)
...
**구조 변경:**
- 10개 절 → 7개 절로 통합
- 파일 열기/파일 핸들 → 파일 열기 (통합)
- 텍스트 파일과 라인 → 파일 읽기 (통합)

**SVG 다이어그램 추가 (8개):**
- `file-arch.svg`: 장 개요 (CPU-RAM-디스크 관계)
- `file-handler.svg`: 파일 핸들 개념
- `file-search-pattern.svg`: 검색 루프 패턴
- `file-whitespace.svg`: 공백 문자 종류와 문자열 구조
- `file-newline.svg`, `file-overview.svg`, `file-read-methods.svg`, `file-try-except.svg`

**callout 박스 추가:**
- `.callout-note`: 공백 문자(Whitespace)란?
- `.callout-warning`: 운영체제별 줄 끝 문자 차이

**윤문 주요 수정:**
- "지금까지, 프로그램을 어떻게 작성하고..." → "앞선 장에서..."
- 각 절 도입 문단 보강 (예외 처리, 파일 쓰기)
- AI 협업 섹션 나열식 → 서술형
- 생각해볼 점 2문단 → 4문단 확장
